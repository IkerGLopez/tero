...
async def _process_question_result(
    ...
    # REQ-001/002/003: read client-level error and parse-failure signals
    client_error = result.get("error", "")
    parse_failures = result.get("parse_failures", 0)

    if parse_failures > 0:
        print(f"  WARNING: {parse_failures} SSE parse failure(s) for "
              f"'{row['question'][:80]}'", flush=True)

    if client_error:
        print(f"  WARNING: client error '{client_error}' for "
              f"'{row['question'][:80]}'", flush=True)
        return {
            **row,
            "error": client_error,
